...
def get_metric_tab_layout(CURR_METRIC,CURR_TITLE, DEFAULT_CAREER = True, DEFAULT_YR = 3): # DEFAULT_CAREER = None, DEFAULT_YR = None
    ...
    fig2_dd = dcc.Dropdown(
        ...
        options = [{'label':'Count','value':'count'}, {'label':'Mean','value':'mean'},
            {'label':'Standard deviation','value':'std'}, {'label':'Minimum','value':'min'},
            {'label':'Maximum','value':'max'}, {'label':'25th percentile','value':'25%'},
            {'label':'50th percentile','value':'50%'}, {'label':'75th percentile','value':'75%'}])
    ...
    @callback(
        Output(CURR_METRIC + 'fig2', 'figure'),
    ...
    def update_figs_metric(career, yr, ns, logTransf,geoColor,CURR_METRIC = CURR_METRIC):
        if career == None or yr == None: return(create_geo_json(df_in = dfs_career[0], df_in_log = dfs_career[0],career = True, yr = 0, metric = CURR_METRIC, empty = True)[0], 'No dataset selected')
        else:
            if ns == True: CURR_METRIC = CURR_METRIC + ' (ns)' 
            if career == True:
                dfs = dfs_career.copy()
