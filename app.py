...
app = Dash(__name__, use_pages=True, external_stylesheets=[dbc.themes.SLATE],
           ...
           external_scripts=[
               ...
               # and zoomed, and in plain canvas they are half the cost of a
               # frame. In WebGL they are close to free.
               #
               # It is an addition rather than a replacement. Everything here
               # still works without it: the map feature-tests for the
               # scatterGL series and falls back to the canvas one when the
               # script has not loaded or the machine has no WebGL, which is
               # the path that can be tested on a build machine.
